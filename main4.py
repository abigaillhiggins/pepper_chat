...
# Add a configurable endpoint at the top
EMOTION_POST_ENDPOINT = os.getenv("EMOTION_POST_ENDPOINT", "http://localhost:5000/emotion")
...
def chat_with_ai(prompt):
    try:
        ...
        print(f"\nResponse generation took {(end - start) * 1000:.2f} ms")
        return response
            ...
            if user_input.lower() != 'quit':
                ...
                for i, sentence in enumerate(sentences, 1):
                    ...
                    if sentence.endswith(('.', '!', '?')):
                        ...
                        # Send HTTP POST with the sentence and its tag
                        try:
                            payload = {"sentence": sentence, "emotion": emotions[0]}
                            resp = requests.post(EMOTION_POST_ENDPOINT, json=payload)
                            print(f"POST to {EMOTION_POST_ENDPOINT}: {resp.status_code}")
                        except Exception as e:
